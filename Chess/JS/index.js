...
for (let i = 0; i < 8; i++) {
    ...
    if (i == 1) {
        for (let index = 0; index < 8; index++) {
            if (index % 2 == 0) {
                document.getElementById(`row${i}`).innerHTML += `<td class="bg-[#a3524e] cursor-pointer	w-14 h-14 sm:w-20 sm:h-20 text-[30px] font-bold p-2" onclick="addData(this)" id="${id}"><img src="./Image/Black_soldier.png"/></td>`;
                id++;
            }
            else if(index % 2 != 0){
                document.getElementById(`row${i}`).innerHTML += `<td class="bg-[#d18b46] cursor-pointer	w-14 h-14 sm:w-20 sm:h-20 text-[30px] font-bold p-2" onclick="addData(this)" id="${id}"><img src="./Image/Black_soldier.png"/></td>`;
                id++;
            }
        }
    }
        ...
        let pieceVriet = piece?piece.replace("_soldier","").replace("_king","").replace("_queen","").replace("horse","").replace("_camel","").replace("_rook",""):alert("Select proper move");
                if (turn == player1) {
                    if(pieceVriet == player1){
                        console.log("player 1 :",pieceVriet);
                        turn = player2;
                    }
                    else{
                    ...
                    if(pieceVriet == player2){
                        console.log("player 2 :",pieceVriet);
                        turn = player1;
                    }
                    else{
